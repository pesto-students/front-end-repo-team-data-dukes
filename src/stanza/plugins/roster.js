...
Strophe.addConnectionPlugin("roster", {
   ...
   */
  get: function (successCallback, errorCallback, ver, items) {
    ...
    return this._connection.sendIQ(
      iq,
      this._onReceiveRosterSuccess.bind(this, successCallback),
      this._onReceiveRosterError.bind(this, errorCallback)
    );
  },
   ...
   *   (Function) call_back
   */
  add: function (jid, name, groups, success_call_back, failure_call_back) {
    var iq = $iq({ type: "set" }).c("query", { xmlns: Strophe.NS.ROSTER }).c("item", { jid: jid, name: name });
    for (var i = 0; i < groups.length; i++) {
      iq.c("group").t(groups[i]).up();
    }
    this._connection.sendIQ(iq, success_call_back, failure_call_back);
  },
  /** Function: update
   * Update roster item
   *
   ...
   *   (Function) call_back
   */
  remove: function (jid, success_call_back, failure_call_back) {
    var item = this.findItem(jid);
    if (!item) {
      throw "item not found";
    }
    var iq = $iq({ type: "set" })
      ...
      .c("item", { jid: item.jid, subscription: "remove" });
    this._connection.sendIQ(iq, success_call_back, failure_call_back);
  },
  ...
  _onReceivePresence: function (presence) {
    ...
    if (type == "unavailable") {
      ...
      item.resources[Strophe.getResourceFromJid(jid)] = {
        show:
          presence.getElementsByTagName("show").length !== 0
            ? Strophe.getText(presence.getElementsByTagName("show")[0])
            : "",
        status:
          presence.getElementsByTagName("status").length !== 0
            ? Strophe.getText(presence.getElementsByTagName("status")[0])
            : "",
        priority:
          presence.getElementsByTagName("priority").length !== 0
            ? Strophe.getText(presence.getElementsByTagName("priority")[0])
            : "",
      };
    } else {
