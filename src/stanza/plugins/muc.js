import { Strophe, $pres, $msg, $iq, $build } from "strophe.js";
/*
...
Strophe.addConnectionPlugin("muc", {
  ...
  /*Function
  Prepare a multi-user chat room
  Parameters:
  (String) room - The multi-user chat room to join.
  (String) nick - The nickname to use in the chat room. Optional
  (Function) on_success_cb - The function call to handle success
  (Function) on_failure_cb - The function call back to handle failure
  in the pre-chat room.
  */
  prep: function (room, nick, on_success_cb, on_failure_cb) {
    var pres, room_nick;
    ...
    return this._connection.sendPresence(pres, on_success_cb, on_failure_cb);
  },
  ...
  delete: function (room, reason, onSuccess, onFailure) {
    var iq = $iq({ type: "set", from: this._connection.jid, to: room });
    iq.c("query", { xmlns: Strophe.NS.MUC_OWNER }).c("destroy", {});
    if (reason) {
      iq.c("reason").t(reason);
    }
    this._connection.sendIQ(iq.tree(), onSuccess, onFailure);
  },
  ...
  Returns:
  msgid - the unique id used to send the invitation
   */
  ...
  getOccupants: function (room, success_cb, error_cb) {
    // Send an IQ stanza to request the participant list
    var iq = $iq({ type: "get", to: room })
      .c("query", { xmlns: Strophe.NS.MUC_ADMIN })
      .c("item", { affiliation: "member" });
    return this._connection.sendIQ(iq, success_cb, error_cb);
  },
  ...
  registrationRequest: function (room, handle_cb, error_cb) {
    ...
    return this._connection.sendIQ(
      iq,
      function (stanza) {
        var $field, $fields, field, fields, i, len;
...
XmppRoom = (function () {
  ...
  XmppRoom.prototype.prep = function (on_success_cb, on_failure_cb) {
    return this.client.prep(this.name, this.nick, on_success_cb, on_failure_cb);
  };
  ...
  XmppRoom.prototype.delete = function (reason, sc_handler_cb, fl_handler_cb) {
    this.client.delete(this.name, reason, sc_handler_cb, fl_handler_cb);
    return delete this.client.rooms[this.name];
  };
  ...
  XmppRoom.prototype.getOccupants = function (success_cb, error_cb) {
    return this.client.getOccupants(this.name, success_cb, error_cb);
  };
  ...
  XmppRoom.prototype.changeNick = function (nick1) {
    this.nick = nick1;
    return this.client.changeNick(this.name, nick1);
  };
